...
def prepare_jsonl(jsonl_path, filename_col, caption_or_tags_col, bucket_col_list, ext_col=None, chunksize=1000):
    ...
    for df in tqdm(pd.read_json(jsonl_path, lines=True, chunksize=chunksize)):
        for idx, data in df.iterrows():
            ...
            metadata = {
                "filename": data[filename_col] + "." + data[ext_col] if ext_col else data[filename_col], 
